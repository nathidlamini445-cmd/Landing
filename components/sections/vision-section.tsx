...
export function VisionSection() {
  return (
    <section id="vision" className="min-h-0 sm:min-h-svh flex items-center justify-center relative py-4 sm:py-8 md:py-12 lg:py-20">
      <div className="container px-4 sm:px-6">
        <motion.div
          ...
          viewport={{ once: true }}
        >
          <h2 className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-sentient font-bold text-slate-900 px-4">Our Vision</h2>
        </motion.div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 sm:gap-8 max-w-6xl mx-auto">
          {visions.map((vision, index) => (
            <motion.div
              key={vision.title}
              className="backdrop-blur-md bg-white/40 border border-blue-200/50 rounded-2xl p-6 sm:p-8 hover:bg-white/60 hover:border-blue-300/60 transition-all duration-300 shadow-lg shadow-blue-500/5"
            ...
            >
              ...
              <h3 className="text-xl sm:text-2xl font-sentient font-semibold text-slate-800 text-center mb-3 sm:mb-4 mt-3 sm:mt-4">
                {vision.title}
              </h3>
              <p className="text-slate-600 text-xs sm:text-sm text-center leading-relaxed">{vision.description}</p>
            </motion.div>
          ))}
        </div>
      </div>
