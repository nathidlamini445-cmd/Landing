  ...
  return (
    ...
    >
      <div className="container mx-auto px-4" style={{ maxWidth: "1100px" }}>
        <SectionHeader
          eyebrow="Explore"
          title="Dive into what matters to you"
          description="Pick a topic to read more or try it out. No endless scrolling."
        />
        ...
        <div className="flex flex-wrap justify-center gap-5">
          {cards.map((card, index) => (
            <Link
              ...
              href={card.href}
              className={[
                "group relative flex flex-col justify-between rounded-[1.25rem] p-6 transition-all duration-200",
                "w-full sm:w-[calc(50%-0.625rem)] lg:w-[calc(33.333%-0.834rem)]",
                card.featured
                  ? "bg-gradient-to-br from-[rgb(0,149,255)] to-blue-700 text-white shadow-xl shadow-blue-500/25 hover:-translate-y-1"
                  : "border border-slate-200/80 bg-white text-slate-900 hover:-translate-y-1 hover:border-[rgba(0,149,255,0.35)] hover:shadow-lg",
              ].join(" ")}
            >
              <span
                className={`absolute right-5 top-5 font-mono text-xs ${
                  card.featured ? "text-blue-100/80" : "text-slate-300"
                }`}
              >
              ...
              <div>
                ...
                <h3 className="font-display text-xl font-medium tracking-tight">{card.title}</h3>
                <p
