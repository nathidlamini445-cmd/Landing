...
export function EcosystemSection() {
  ...
  return (
    <section ref={sectionRef} id="ecosystem" className="relative min-h-0 sm:min-h-screen flex items-start justify-center pt-4 sm:pt-6 md:pt-8 pb-6 sm:pb-8 md:pb-12 lg:pb-24">
      <div className="container mx-auto px-4 sm:px-6 mt-4 sm:mt-8 md:mt-12 lg:mt-16">
        <div
          className={`text-center mb-4 sm:mb-6 md:mb-8 transition-all duration-1000 ${
            isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"
          }`}
        >
          <h2 className="text-3xl sm:text-4xl md:text-5xl font-bold text-slate-900 mb-2 sm:mb-4 md:mb-6 px-4">Introducing Our EcoSystem</h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 sm:gap-8 max-w-5xl mx-auto">
          {products.map((product, index) => (
            ...
            >
              ...
              <h3 className="text-xl sm:text-2xl font-bold text-slate-800 mb-3 sm:mb-4 text-center">{product.title}</h3>
              <p className="text-slate-600 text-center leading-relaxed text-sm sm:text-base">{product.description}</p>
            </div>
          ))}
        </div>
      </div>
