  ...
  constructor() {
    super({
      ...
      void main() { 
        ...
        gl_PointSize = max(vDistance * uBlur * uPointSize, 5.5);
      }`,
      fragmentShader: /* glsl */ `
      ...
      void main() {
        ...
        float alphaRaw = (1.04 - clamp(vDistance, 0.0, 1.0)) * clamp(smoothstep(-0.5, 0.35, vPosY), 0.0, 1.0) * uOpacity * revealMask * uRevealProgress * sparkleBrightness;
        float alpha = min(alphaRaw * 1.35, 1.0);

        vec3 deepBlue = vec3(0.0, 0.35, 0.85);
        vec3 vibrantBlue = vec3(0.1, 0.55, 1.0);
        vec3 brightBlue = vec3(0.25, 0.72, 1.0);
        
        // Mix between the blue tones based on position and sparkle
        float colorMix = smoothstep(0.0, 1.0, sparkleBrightness - 0.3 + vPosY * 0.4);
        vec3 particleColor = mix(deepBlue, mix(vibrantBlue, brightBlue, colorMix * 0.5), colorMix);

        vec3 vivid = particleColor * 1.12;
        gl_FragColor = vec4(vivid, mix(alpha, sparkleBrightness - 1.1, uTransition));
      }`,
      uniforms: {
